   ...
   boolean isConnectionAlive(final Connection connection)
   {
      try {
         try {
            ...
            // validationTimeout默认5s，所以validationSeconds的默认范围是1~5s
            // validationTimeout <connectionTimeout(30s),所以在不调整connectionTimeout的情况下validationSeconds峰值为30s
            final int validationSeconds = (int) Math.max(1000L, validationTimeout) / 1000;

            if (isUseJdbc4Validation) {// jdbc4,是ping命令检测，性能更高
