...
public final class HikariPool extends PoolBase implements HikariPoolMXBean, IBagStateListener
{
   ...
   /**
    * 用于创建物理连接池，使用DiscardPolicy策略，
    * 默认情况下会丢弃被拒绝的任务，
    * 实际上就是任务满了就不会抛出异常
    */
   private final ThreadPoolExecutor addConnectionExecutor;
   /**
    * 使用CallerRunsPolicy策略，如果添加到线程池失败，主线程会直接在
    * execute方法的调用线程中运行被拒绝的任务，若执行程序已关闭，则会丢弃该任务
    */
   ...
   public Connection getConnection(final long hardTimeout) throws SQLException
   {
      ...
      try {
         ...
         do {
        	/**
        	 * - 从connectionBag中借用连接，借用过程中会发生创建连接、连接过期、空闲等事情
        	 */
            ...
            if (poolEntry.isMarkedEvicted() || 
            		(elapsedMillis(poolEntry.lastAccessed, now) > aliveBypassWindowMs 
            				&& !isConnectionAlive(poolEntry.connection))) {// 有效性检测
         ...
         } while (timeout > 0L); // 只要超时时间大于0，就需要不断重试
   ...
   public synchronized void shutdown() throws InterruptedException
   {
      try {
         ...
         if (houseKeeperTask != null) {// close house keepper
            houseKeeperTask.cancel(false);
            houseKeeperTask = null;
         }

         softEvictConnections();// 软驱逐

         addConnectionExecutor.shutdown();// 关闭增加连接池
   ...
   void closeConnection(final PoolEntry poolEntry, final String closureReason)
   {
      if (connectionBag.remove(poolEntry)) {
         ...
         closeConnectionExecutor.execute(() -> {
            ...
            /**
             * 如果poolState的状态为0，
             * 还会使用另一个独立线程池addConnectionExecutor
             * （与closeConnectionExecutor对称，
             * 本质上也是ThreadPoolExecutor）进行新连接的生成，
             * fillPool补足到HikariCP的配置值
             */
